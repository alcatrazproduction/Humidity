...
#define spi_read() spi_RW_End(0x00)
...
INT8U i,nn = n;
	...
	for (i=0; i<=nn; i++) 
    	{ 
		values[i] = i < nn ? spi_RW(0) : spi_read();
	}
	MCP2515_UNSELECT();
}
...
INT8U MCP_CAN::mcp2515_init(const INT8U canSpeed)                       /* mcp2515init                  */
{
  ...
  INT8U res;
    ...
    if(res > 0)
    {
	printf("error entering setting mode %d\n",res);
      return res;
    }

                                                                        /* set boadrate                 */
    if( (res = mcp2515_configRate(canSpeed)) != 0 )
    {
        printf("error setting rate  %d\n",res);
      return res;
    }
...
//        mcp2515_setRegister( MCP_CANINTE, MCP_RX0IF | MCP_RX1IF);
        mcp2515_setRegister( MCP_CANINTE, 0 );				// do not use interupt for testing...
