...
void CAN_MCP2515::begin(uint32_t bitrate, uint8_t mode)
{
  ...
  SPI.setDataMode( SPI_MODE3 );		// Set mode
  SPI.setBitOrder( MSBFIRST );		// most first ?
  SPI.setClockDivider( SPI_CLOCK_DIV128 );     // clock divide
...
PRINTF( stdout, "setting CAN: %x,%x,%x       \n",readAddress(MCP2515_CNF1),readAddress(MCP2515_CNF2),readAddress(MCP2515_CNF3));
...
CAN_Frame CAN_MCP2515::read()
{
  ...
  digitalWrite(CS, LOW);
  SPI.transfer(buffer, SPI_CONTINUE);
  RXBnSIDH = SPI.transfer(0xFF, SPI_CONTINUE); // SID<10:3>
  RXBnSIDL = SPI.transfer(0xFF, SPI_CONTINUE); // SID<2:0>, SRR, IDE, EID<17:16>
  RXBnEID8 = SPI.transfer(0xFF, SPI_CONTINUE); // EID<15:8>
  RXBnEID0 = SPI.transfer(0xFF, SPI_CONTINUE); // EID<7:0>
  RXBnDLC  = SPI.transfer(0xFF, SPI_CONTINUE); // RTR, RB<1:0>, DLC<3:0>
  message.length = (RXBnDLC & MCP2515_DLC);
  for (int i = 0; i < message.length; i++)
  {
    message.data[i] = SPI.transfer(0xFF, i < (message.length - 1) ? SPI_CONTINUE : SPI_LAST);
  }
  digitalWrite(CS, HIGH);
 //**** to test: TODO:
  if (buffer == MCP2515_READ_RX_BUFFER_0_ID)
  {
	modifyAddress( MCP2515_CANINTF, MCP2515_RX0IF, 0 );
  }
  else
  {
    modifyAddress( MCP2515_CANINTF, MCP2515_RX1IF, 0 );
  }
...
uint8_t CAN_MCP2515::write(const CAN_Frame & message)
{
  ...
  digitalWrite(CS, LOW);
  SPI.transfer(loadBuffer, 	SPI_CONTINUE );
  SPI.transfer(TXBnSIDH, 	SPI_CONTINUE ); //ID high bits
  SPI.transfer(TXBnSIDL,	SPI_CONTINUE ); //ID low bits
  SPI.transfer(TXBnEID8,	SPI_CONTINUE ); //extended ID high bits
  SPI.transfer(TXBnEID0,	SPI_CONTINUE ); //extended ID low bits
  SPI.transfer(TXBnDLC,		SPI_CONTINUE ); //data length code
  for (int i = 0; i < message.length; i++)   //load data buffer
  {
    SPI.transfer(message.data[i], i < (message.length - 1) ? SPI_CONTINUE : SPI_LAST );
  }
  digitalWrite(CS, HIGH);
  digitalWrite(CS, LOW);
  SPI.transfer(sendBuffer, SPI_LAST );
  digitalWrite(CS, HIGH);
...
{
  digitalWrite(CS, LOW);
  SPI.transfer(MCP2515_SPI_RESET, SPI_LAST );
  digitalWrite(CS, HIGH);
}
...
{
  digitalWrite(CS, LOW);
  SPI.transfer(MCP2515_SPI_READ, SPI_CONTINUE );
  SPI.transfer(address, SPI_CONTINUE );
  uint8_t retVal = SPI.transfer(0xFF, SPI_LAST );
  digitalWrite(CS, HIGH);
  return retVal;
}
...
{
  digitalWrite(CS, LOW);
  SPI.transfer(MCP2515_SPI_WRITE, SPI_CONTINUE );
  SPI.transfer(address, SPI_CONTINUE );
...
{
  digitalWrite(CS, LOW);
  SPI.transfer(MCP2515_SPI_BIT_MODIFY, SPI_CONTINUE );
  SPI.transfer(address, SPI_CONTINUE );
  SPI.transfer(mask, SPI_CONTINUE);
  SPI.transfer(value, SPI_LAST);
  digitalWrite(CS, HIGH);
}
...
{
  digitalWrite(CS, LOW);
  SPI.transfer(MCP2515_SPI_READ_STATUS, SPI_CONTINUE );
  uint8_t retVal = SPI.transfer(0xFF, SPI_LAST );
  digitalWrite(CS, HIGH);
  return retVal;
}
...
{
  digitalWrite(CS, LOW);
  SPI.transfer(MCP2515_SPI_RX_STATUS, SPI_CONTINUE );
  uint8_t retVal = SPI.transfer(0xFF, SPI_LAST );
  digitalWrite(CS, HIGH);
  return retVal;
  /*
...
void CAN_MCP2515::setMask(uint8_t mask, uint8_t b0, uint8_t b1, uint8_t b2, uint8_t b3)
{
  ...
  SPI.transfer(b1,   SPI_CONTINUE );
  SPI.transfer(b2,   SPI_CONTINUE );
  SPI.transfer(b3,   SPI_LAST );
  digitalWrite(CS, HIGH);
...
void CAN_MCP2515::setFilter(uint8_t filter, uint8_t b0, uint8_t b1, uint8_t b2, uint8_t b3)
{
  setMode(MCP2515_MODE_CONFIG);
  digitalWrite(CS, LOW);
  SPI.transfer(filter, SPI_CONTINUE );
  SPI.transfer(b0,     SPI_CONTINUE );
  SPI.transfer(b1,     SPI_CONTINUE );
  SPI.transfer(b2,     SPI_CONTINUE );
  SPI.transfer(b3,     SPI_LAST);
...
void CAN_MCP2515::clearRxBuffers()
{
  digitalWrite(CS, LOW);
  SPI.transfer(MCP2515_SPI_WRITE, SPI_CONTINUE );
  SPI.transfer(MCP2515_RXB0SIDH,  SPI_CONTINUE);
  ...
  digitalWrite(CS, LOW);
  SPI.transfer (MCP2515_SPI_WRITE, SPI_CONTINUE );
  SPI.transfer (MCP2515_RXB1SIDH,  SPI_CONTINUE );
  for (uint8_t i = 0; i < 12; i++)
  {
    SPI.transfer(0x00, SPI_CONTINUE );
  }
...
void CAN_MCP2515::clearTxBuffers()
{
  digitalWrite(CS, LOW);
  SPI.transfer (MCP2515_SPI_WRITE, SPI_CONTINUE );
  SPI.transfer (MCP2515_TXB0SIDH,  SPI_CONTINUE );
  for (uint8_t i = 0; i < 12; i++)
  {
    SPI.transfer(0x00, SPI_CONTINUE );
  }
  ...
  digitalWrite(CS, LOW);
  SPI.transfer (MCP2515_SPI_WRITE, SPI_CONTINUE );
  SPI.transfer (MCP2515_TXB1SIDH,  SPI_CONTINUE );
  for (uint8_t i = 0; i < 12; i++)
  {
    SPI.transfer(0x00, SPI_CONTINUE );
  }
  ...
  digitalWrite(CS, LOW);
  SPI.transfer (MCP2515_SPI_WRITE, SPI_CONTINUE );
  SPI.transfer (MCP2515_TXB2SIDH,  SPI_CONTINUE );
  for (uint8_t i = 0; i < 12; i++)
  {
    SPI.transfer(0x00, SPI_CONTINUE );
  }
