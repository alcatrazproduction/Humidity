...
class CANClass // Can't inherit from Stream
{
  public:
    ...
    virtual void end() {};
    virtual uint8_t available() { return false;};
    virtual CAN_Frame read() { CAN_Frame f; f.valid = false; f.length=0; return f; };
    virtual void flush() {};
    virtual uint8_t write(const CAN_Frame&) { return 0; };
